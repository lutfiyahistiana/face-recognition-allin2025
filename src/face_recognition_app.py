...
class FaceRecognitionModel:
    def __init__(self):
        ...
        self.min_percentage = 30.0 # Treshold
    ...
    def set_min_percentage(self, min_percentage):
        """Mengatur nilai minimum persentase untuk recognition"""
    ...
    def save_cache(self, path):
        ...
        np.savez(path, u=self.u, wdata=self.wdata, rawimg=self.rawimg, filesname=self.filesname, count=self.count, data=self.data, k=self.k, min_percentage=self.min_percentage)
    ...
    def load_cache(self, dataset_path, cache_path):
        ...
        if 'min_percentage' in data:
            self.min_percentage = float(data['min_percentage'])
        else:
            self.min_percentage = 30.0 # Default jika ngga ada di cache lama
    
    def set_min_percentage(self, min_percentage_value):
        """
        Mengatur nilai threshold untuk recognition
        """
        self.min_percentage = min_percentage_value
...
class FaceRecognitionApp:
    ...
    def setup_ui(self):
        ...
        # Komponen ui
        self.setup_komponen_ui()
    
    def setup_komponen_ui(self):
        ...
        self.chooseDatasetBtn.place(x=60, y=222, width=150, height=50)
        ...
        self.chooseImgBtn.place(x=60, y=332, width=150, height=50)
        ...
        self.startImgBtn.place(x=50, y=513, width=388, height=70)
        ...
        self.videoBtn.place(x=50, y=583, width=388, height=70)
        ...
        try:
            ...
            self.canvas.create_image(324, 357, image=self.entry2_img)
        except:
            pass
        ...
        # Frame treshold container
        self.entry1_img = PhotoImage(file="assets/tresholdBg.png")
        self.canvas.create_image(244, 467, image=self.entry1_img)
        threshold_frame = Frame(self.window, bg='#1146a2', relief='solid', bd=0)
        threshold_frame.place(x=85, y=442, width=318, height=50)
        
        # Persentase kemiripan minimum
        self.treshold_var = DoubleVar(value=30.0)  # Default value
        self.threshold_scale = Scale(
            threshold_frame,
            from_=0, to=100, resolution=1,  # Range 0-100 dengan step 1
            orient=HORIZONTAL,
            variable=self.treshold_var,
            command=self.update_min_percentage,
            length=318,
            bg='#1146a2',
            fg='#ffffff',
            highlightbackground='#ffffff',
            troughcolor='#ffffff',
            activebackground='#f70044',
            font=('Poppins', 10),
            relief='flat',
            bd=0
        )
        self.threshold_scale.pack(side=LEFT, padx=0, pady=0)
        ...
        # Frame treshold perentase
        self.entry1_img = PhotoImage(file="assets/tresholdBg.png")
        self.canvas.create_image(244, 467, image=self.entry1_img)
        min_percentage_frame = Frame(self.window, bg='#1146a2', relief='solid', bd=0)
        min_percentage_frame.place(x=85, y=442, width=318, height=50)
        
        # Persentase kemiripan minimum
        self.min_percentage_var = DoubleVar(value=30.0)  # Default value
        self.min_percentage_scale = Scale(
            min_percentage_frame,
            from_=0, to=100, resolution=1,  # Range 0-100 dengan step 1
            orient=HORIZONTAL,
            variable=self.treshold_var,
            command=self.update_min_percentage,
        ...
        self.min_percentage_scale.pack(side=LEFT, padx=0, pady=0)

    def update_min_percentage(self, value):
        """Update minimum percentage value"""
        min_percentage = float(value)
        self.model.set_min_percentage(min_percentage)
    ...
    def start_recognition(self):
        ...
        if result['recognized']:
            ...
            # Gambar default untuk unknown person
            unknown_img = PhotoImage(file="assets/unknownBg.png")  # Buat gambar unknown
            self.canvas.create_image(764, 227, anchor=NW, image=unknown_img)
            self.canvas.image = unknown_img
            
            # Update labels untuk unknown person
            self.result_label.config(text="Unknown Person")
            self.percentage_label.config(text="0.0%")
    ...
    def start_video(self):
        ...
        threading.Thread(target=self.loop_video, daemon=True).start()
